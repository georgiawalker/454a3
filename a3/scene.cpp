...

{
  ...
  if (g == 1 || glossyIterations == 1) {
    ...
    // YOUR CODE HERE
	  ...
	  for (int i = 0; i < glossyIterations; i++) {
		  ...
		  float randb;
		  do {
			  randa = 2 * static_cast <float>(rand()) / RAND_MAX - 1;
			  randb = 2 * static_cast <float>(rand()) / RAND_MAX - 1;
		  } while (randa * randa + randb * randb > 1.0);

		  vec3 Rprime = (l * R + randa * u + randb * v).normalize();

		  vec3 Iin = raytrace(P, Rprime, depth, objIndex, objPartIndex);

		 
		  glossyIout = glossyIout + calcIout(N, Rprime, E, E, vec3(0.0, 0.0, 0.0), mat->ks, mat->n, Iin);
	  }
  ...
  if (opacity < 1.0) { // not completely opaque
	...
	// Use the 'findRefractionDirection' function (below).
	  vec3 refracted_rayDir;
	  findRefractionDirection(Iout, N, refracted_rayDir);
	  
	  vec3 i1 = opacity * Iout;	//multiply opacity with magnitude of i1
	  vec3 i2 = (1 - opacity) * refracted_rayDir;

	  return i1 + i2; // set this eaqual to Iout?
  }
...

{
  ...
  // YOUR CODE HERE
	float IofR_air = 1.008;
	float IofR_glass = 1.510;

	float RdotN = rayDir.x * N.x + rayDir.y * N.y + rayDir.z * N.z;
	float sin_theta_glass;
	float sin_theta_air;
	float theta_glass;
	float theta_air;
	if (RdotN > 0) {	//ray entering surface
		//air to glass transition
		sin_theta_air = rayDir.x / (sqrt((rayDir.x * rayDir.x) + (rayDir.y * rayDir.y)));
		sin_theta_glass = sin_theta_air * (IofR_air/IofR_glass);
		theta_glass = asin(sin_theta_glass);
		refractionDir = vec3(sin_theta_glass, cos(theta_glass), 0);
		return true;
	}
	else if (RdotN < 0) {				//ray leaving surface
		sin_theta_glass = rayDir.x / (sqrt((rayDir.x * rayDir.x) + (rayDir.y * rayDir.y)));
		sin_theta_air = sin_theta_glass * (IofR_glass / IofR_air);
		theta_air = asin(sin_theta_air);
		refractionDir = vec3(sin_theta_air, cos(theta_air), 0);
		return true;
	}
	else {
		return false;
...

{
  ...
  int numRays = numPixelSamples * numPixelSamples;
  vec3 colourDir = vec3(0, 0, 0);
  ...
  //check if using jittered pattern or not:
  if (jitter == true) {
	  //loop 
	  for (int i = 0; i < numPixelSamples; i++) {
		  for (int j = 0; j < numPixelSamples; j++) {
			  //get lowerleft of jitter square
			  lowerleft = vec3(j * (1 / numPixelSamples), i*(1/numPixelSamples), 1);	//check that 0 z coord is okay here?
			  
			  //get random x val to add to lowerleft
			  randtemp = rand() % 1000;
			  randtemp = randtemp / 1000;
			  randx = randtemp * (1 / numPixelSamples);
			  
			  //get random y val to add to lowerleft
			  randtemp = rand() % 1000;
			  randtemp = randtemp / 1000;
			  randy = randtemp * (1 / numPixelSamples);
			  
			  //define direction vector
			  colourDir = vec3(lowerleft + (x + randx) * right + (y + randy) * up).normalize();
			  
			  //call ray tracer
			  tempColour = raytrace(eye->position, colourDir, 0, -1, -1);

			  //add colour to colour sum
			  colourSum = colourSum + tempColour;
		  }
	  }
  }
  else {
	  ...
	  for (int i = 0; i < numRays; i++) {
		  //get random x val to add to lower left
		  randtemp = rand() % 1000;
		  randx = randtemp / 1000;

		  //get random y val to add to lower left
		  randtemp = rand() % 1000;
		  randy = randtemp / 1000;
		  
		  //define direction vector
		  colourDir = vec3(llCorner + (x + randx) * right + (y + randy) * up).normalize();
  ...
  //get colour avg
  vec3 colourAvg = vec3(colourSum.x / numRays, colourSum.y / numRays, colourSum.z / numRays);
  result = colourAvg;
